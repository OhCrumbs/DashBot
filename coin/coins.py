...
class Coins(commands.Cog):
    ...
    async def coin(self, ctx: commands.Context):
        ...
        if cur_time >= next_coin:
            ...
            await ctx.send(f"Here is your {amount} <:dashcoin:816398765829128233>")
        else:
    ...
    async def steal(self, ctx: commands.Context, target: discord.Member = None):
        ...
        if target_coins == 0:
            return await ctx.send(
                f"Uh oh, {target.display_name} doesn't have any <:dashcoin:816398765829128233>"
            )
        ...
        if success_chance > 90:
            ...
            target_coins -= stolen
            await ctx.send(f"You stole {stolen} <:dashcoin:816398765829128233> from {target.display_name}!")
        else:
            ...
            if coins_penalty > 0:
                ...
                if self._max_balance_check(target_coins + penalty):
                    return await ctx.send(
                        f"Uh oh, you got caught while trying to steal {target.display_name}'s <:dashcoin:816398765829128233>\n"
                ...
                await ctx.send(
                    f"You got caught while trying to steal {target.display_name}'s <:dashcoin:816398765829128233>\nYour penalty is {penalty} <:dashcoin:816398765829128233> which they got!"
                )
            else:
                return await ctx.send(
                    f"Uh oh, you got caught while trying to steal {target.display_name}'s <:dashcoin:816398765829128233>\n"
    ...
    async def gift(self, ctx: commands.Context, target: discord.Member, amount: int):
        ...
        await self.config.member(target).coins.set(target_coins)
        await ctx.send(
            f"{ctx.author.mention} has gifted {amount} <:dashcoin:816398765829128233> to {target.mention}"
        )
    ...
    async def coin(self, ctx: commands.Context, target: discord.Member = None):
        ...
        if not target:
            coins = int(await self.config.member(ctx.author).coins())
            await ctx.send(f"You have {coins} <:dashcoin:816398765829128233>")
        else:
            coins = int(await self.config.member(target).coins())
            await ctx.send(f"{target.display_name} has {coins} <:dashcoin:816398765829128233>")
    ...
    async def coinexchange(self, ctx: commands.Context, amount: int):
        ...
        await ctx.send(f"You have exchanged {amount} {currency} and got {new_coins} <:dashcoin:816398765829128233>\nYou now have {coins} <:dashcoin:816398765829128233>")
    ...
    async def setcoins_set(
        ...
        await ctx.send(f"Set {target.mention}'s balance to {amount} <:dashcoin:816398765829128233>")
    ...
    async def setcoins_add(
        ...
        await self.config.member(target).coins.set(target_coins)
        await ctx.send(f"Added {amount} <:dashcoin:816398765829128233> to {target.mention}'s balance.")
    ...
    async def setcoins_take(
        ...
        if amount <= target_coins:
            ...
            await self.config.member(target).coins.set(target_coins)
            await ctx.send(
                f"Took away {amount} <:dashcoin:816398765829128233> from {target.mention}'s balance."
            )
        else:
    ...
    async def setcoins_rate(self, ctx: commands.Context, rate: Union[int, float]):
        ...
        await ctx.send(f"Set the exchange rate {rate}. This means that 100 {currency} will give you {test_amount} <:dashcoin:816398765829128233>")
    ...
    async def setcoins_role_add(
        ...
        await ctx.send(f"Gaining {role.name} will now give {amount} <:dashcoin:816398765829128233>")
    ...
    async def setcoins_role_del(self, ctx: commands.Context, role: discord.Role):
        ...
        await ctx.send(f"Gaining {role.name} will now not give any <:dashcoin:816398765829128233>")
    ...
    async def setcoins_role_show(self, ctx: commands.Context, role: discord.Role):
        ...
        await ctx.send(f"Gaining {role.name} gives {coins} <:dashcoin:816398765829128233>")
    ...
    async def setcoins_role_multiplier(
        ...
        await ctx.send(f"Users with {role.name} will now get {multiplier} times more <:dashcoin:816398765829128233>")
