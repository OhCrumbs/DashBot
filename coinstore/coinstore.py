...
class CoinStore(commands.Cog):
    """
    Coin store add-on for coin feature.
    """
    ...
    async def store_add(self, ctx: commands.Context):
        ...
        if pred.result == 0:
            ...
            try:
                ...
                await ctx.send("How many coins should this item be?")
                try:
            ...
            try:
                ...
                await ctx.send("How many coins should this role be?")
                try:
                    await self.bot.wait_for("message", timeout=120, check=pred_int)
                except asyncio.TimeoutError:
                    return await ctx.send("You took too long. Try again, please.")
            ...
            try:
                ...
                await ctx.send("How many coins should this game be?")
                try:
                    await self.bot.wait_for("message", timeout=120, check=pred_int)
                except asyncio.TimeoutError:
                    return await ctx.send("You took too long. Try again, please.")
    ...
    async def buy(self, ctx: commands.Context, *, item: str = ""):
        ...
        if not enabled:
            return await ctx.send("Uh oh, store is disabled.")
        coins = int(
            await self.bot.get_cog("Coins").config.member(ctx.author).coins()
        )
        items = await self.config.guild(ctx.guild).items.get_raw()
        roles = await self.config.guild(ctx.guild).roles.get_raw()
        games = await self.config.guild(ctx.guild).games.get_raw()
        ...
        if item in roles:
            ...
            if role_obj:
                ...
                coins -= price
                quantity -= 1
                await self.bot.get_cog("Coins").config.member(ctx.author).coins.set(
                    coins
                )
    ...
    async def store_return(self, ctx: commands.Context, *, item: str):
        ...
        await self.bot.get_cog("Coins").config.member(ctx.author).coins.set(coins)
        await ctx.send(
            f"You have returned {item} and got {return_price} <:dashcoin:816398765829128233> back."
        )
    ...
    async def _show_store(self, ctx):
        ...
        for i in items:
            ...
            item_text = f"__Item:__ **{i}** | __Price:__ {price} <:dashcoin:816398765829128233> | __Quantity:__ {quantity}"
        ...
        for g in games:
            ...
            game_text = f"__Item:__ **{g}** | __Price:__ {price} <:dashcoin:816398765829128233> | __Quantity:__ {quantity}"
        ...
        for r in roles:
            ...
            price = int(role.get("price"))
            quantity = int(role.get("quantity"))
            role_text = f"__Role:__ **{role_obj.mention}** | __Price:__ {price} <:dashcoin:816398765829128233> | __Quantity:__ {quantity}"
        ...
        for page in pagify(desc, delims=["\n"], page_length=1000):
            ...
            embed.set_author(
                name=f"{ctx.guild.name}'s coin store", icon_url=ctx.guild.icon_url,
            )
